...
public class HttpService
{
    ...
    public async Task<IEnumerable<SshFolder>?> GetFolder(string ip,string username, string password)
    {
        var response = await _client.GetAsync($"Ssh/GetFolders?Username={username}&Password={password}&Ip={ip}");
        var text = await response.Content.ReadAsStringAsync();
